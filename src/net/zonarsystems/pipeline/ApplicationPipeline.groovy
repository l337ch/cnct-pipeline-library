...
class ApplicationPipeline implements Serializable {
  ...
  def checkImageForNewPackageVersion(dockerImage, packageName) {
    ...
    def currentVersion = getSteps().sh(script: "docker run -it ${dockerImage} yum list installed ${packageName} | awk \'END {print \$2 }\'", returnStdout: true)
    def availableVersion = getSteps().sh(script: "docker run -it ${dockerImage} yum list available ${packageName} | awk \'END {print \$2 }\'", returnStdout: true)
