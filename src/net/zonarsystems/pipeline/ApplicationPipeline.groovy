...
class ApplicationPipeline implements Serializable {
    ...
    def getPipeline() {
        bailOnUninitialized(); pipeline[application]
    }

    def getPipeline(app) {
        bailOnUninitialized(); pipeline[app]
    }
    ...
    def bailOnUninitialized() {
        if (!this.ready) {
            throw new Exception('Pipeline not initialized,run init() first')
        }

}

ApplicationPipeline(steps,application,script,overrides=[:],e2e=[:]){this.steps=steps this.application=application this.script=script this.e2e=e2e this.overrides=overrides}
...
def pipelineCheckout() {
    ...
    getSteps().stage('Checkout'){if(getEnvironment().CHANGE_ID!=null){getHelpers().githubPRCheckout(application,getEnvironment().CHANGE_ID)}else{if(getEnvironment().BRANCH_NAME!='master'){getSteps().error'Aborting non-master branch build'
}
...
def getZonarAppVersionOverrides() {
    ...
    def e2eTestHelmCharts(namespace, releaseName) {
        ...
        getSteps().stage ('Helm chart(s)end to end testing') {
            ...
            for(def i=0;i<chartsFolders.size();i++){
                if(getSteps().fileExists("${chartsFolders[i]}/Chart.yaml")){
                    deployHelmChartsFromPath(chartsFolders[i],'staging',releaseName)
    ...
    def deleteHelmRelease(releaseName) {
        bailOnUninitialized()getSteps().stage('Delete helm release'){
    ...
    def getHelmChartValue(directory,property) {
        bailOnUninitialized()
        def valuesYaml=getScript().parseYaml{
            yaml=getSteps().readFile("${directory}/Chart.yaml")
            }
            return valuesYaml.getProperties().get(property)
    }
    ...
    def updateChartVersionMetadata(sha) {
        ...
        getSteps().stage ('Increment chart(s)version(s)') {
            ...
            for(def i=0;i<chartsFolders.size();i++){
                ...
                def verComponents=[]verComponents.addAll(chartYaml.version.toString().split(Pattern.quote('+')))

                if(verComponents.size()>1){verComponents[1]=sha}else{verComponents<<sha}
    ...
    def init() {
        getSteps().podTemplate(label:"env-${application}",containers:[getSteps().containerTemplate(name:'jnlp',image:'jenkinsci/jnlp-slave:2.62-alpine', args: '${computer.jnlpmac}${computer.name}'),],volumes:[]){getSteps().node("env-${application}"){this.settings=getFileLoader().fromGit('settings','https://github.com/samsung-cnct/zonar-pipeline.git','master','repo-scan-access','').getConfig()
    ...
    def getChartOverrides(allOverrides,chart,type) {
        ...
        if(allOverrides){def chartOverrides=allOverrides.get(chart)
            if(chartOverrides){def chartTypeOverrides=chartOverrides.get(type)
                if(chartTypeOverrides){res=chartTypeOverrides.inject([]){
                    result,entry->result<<"${entry.key}=${entry.value.toString()}"}.join(',')
                    }
                    }

            def zonarVersionOverrides=getZonarAppVersionOverrides()
            if(zonarVersionOverrides){res=res+zonarVersionOverrides.inject([]){
                result,entry->result<<"${entry.key}=${entry.value.toString()}"}.join(',')
                }
    ...
    def pipelineRun() {
        ...
        getSteps().podTemplate(label:"CI-${application}",containers:[getSteps().containerTemplate(name:'jnlp',image:'jenkinsci/jnlp-slave:2.62-alpine',args:'${computer.jnlpmac}${computer.name}'),getSteps().containerTemplate(name:'gke',image:"${getSettings().dockerRegistry}/jenkins-gke:latest",ttyEnabled:true,command:'cat',alwaysPullImage:true),],volumes:[getSteps().hostPathVolume(hostPath:'/var/run/docker.sock',mountPath:'/var/run/docker.sock'),getSteps().secretVolume(mountPath:'/etc/gke',secretName:"jenkins-gke-${uniqueJenkinsId}"),getSteps().secretVolume(mountPath:'/etc/helm',secretName:"jenkins-helm-${uniqueJenkinsId}")]){
            ...
            getSteps().node("CI-${application}"){
                ...
                try{
                    ...
                    getSteps().container('gke'){
                        ...
                        // build docker containers and push them with current SHA tag and
                        // 'latest'
                        buildContainersWithTag(getScript().getGitSha())pushContainersWithTag(getScript().getGitSha())tagContainers(getScript().getGitSha(),'latest')pushContainersWithTag('latest')
                        ...
                        if(getEnvironment().CHANGE_ID){
                            ...
                            tagContainers(getScript().getGitSha(),STAGING_TAG)pushContainersWithTag(STAGING_TAG)
